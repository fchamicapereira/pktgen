...
static int find_key(int *busybits, void **keyps, unsigned *k_hashes, int *chns, void *keyp, map_keys_equality *eq, unsigned key_hash,
                    unsigned capacity) {
...
static unsigned find_key_remove_chain(int *busybits, void **keyps, unsigned *k_hashes, int *chns, void *keyp, map_keys_equality *eq,
...
int map_impl_get(int *busybits, void **keyps, unsigned *k_hashes, int *chns, int *values, void *keyp, map_keys_equality *eq, unsigned hash,
                 int *value, unsigned capacity) {
  int index = find_key(busybits, keyps, k_hashes, chns, keyp, eq, hash, capacity);
  if (-1 == index) {
    return 0;
  }
...
void map_impl_erase(int *busybits, void **keyps, unsigned *k_hashes, int *chns, void *keyp, map_keys_equality *eq, unsigned hash,
                    unsigned capacity, void **keyp_out) {
  find_key_remove_chain(busybits, keyps, k_hashes, chns, keyp, eq, hash, capacity, keyp_out);
}
...
int map_allocate(map_keys_equality *keq, map_key_hash *khash, unsigned capacity, struct Map **map_out) {
  struct Map *old_map_val = *map_out;
  struct Map *map_alloc   = (struct Map *)rte_malloc(NULL, sizeof(struct Map), 64);
  if (map_alloc == NULL)
    return 0;
...
int map_get(struct Map *map, void *key, int *value_out) {
  ...
  return map_impl_get(map->busybits, map->keyps, map->khs, map->chns, map->vals, key, map->keys_eq, hash, value_out, map->capacity);
}
...
void map_erase(struct Map *map, void *key, void **trash) {
  map_key_hash *khash = map->khash;
  unsigned hash       = khash(key);
  map_impl_erase(map->busybits, map->keyps, map->khs, map->chns, key, map->keys_eq, hash, map->capacity, trash);
...
int dchain_allocate(int index_range, struct DoubleChain **chain_out) {
  ...
  struct DoubleChain *chain_alloc   = (struct DoubleChain *)rte_malloc(NULL, sizeof(struct DoubleChain), 64);
  if (chain_alloc == NULL)
    return 0;
  *chain_out = (struct DoubleChain *)chain_alloc;

  struct dchain_cell *cells_alloc =
      (struct dchain_cell *)rte_malloc(NULL, sizeof(struct dchain_cell) * (index_range + DCHAIN_RESERVED), 64);
  if (cells_alloc == NULL) {
  ...
  time_t *timestamps_alloc = (time_t *)rte_malloc(NULL, sizeof(time_t) * (index_range), 64);
...
int dchain_allocate_new_index(struct DoubleChain *chain, int *index_out, time_t time) {
...
int dchain_expire_one_index(struct DoubleChain *chain, int *index_out, time_t time) {
...
int vector_allocate(int elem_size, unsigned capacity, vector_init_elem *init_elem, struct Vector **vector_out) {
  struct Vector *old_vector_val = *vector_out;
  struct Vector *vector_alloc   = (struct Vector *)rte_malloc(NULL, sizeof(struct Vector), 64);
  if (vector_alloc == 0)
    return 0;
...
void vector_get(struct Vector *vector, int index, void **val_out) { *val_out = vector->data + index * vector->elem_size; }

int expire_items_single_map(struct DoubleChain *chain, struct Vector *vector, struct Map *map, time_t time) {
  int count = 0;
  int index = -1;
...
void expire_items_single_map_iteratively(struct Vector *vector, struct Map *map, int start, int n_elems) {
  assert(start >= 0);
  assert(n_elems >= 0);
  void *key;
...
static const uint32_t SKETCH_SALTS[SKETCH_SALTS_BANK_SIZE] = {
    0x9b78350f, 0x9bcf144c, 0x8ab29a3e, 0x34d48bf5, 0x78e47449, 0xd6e4af1d, 0x32ed75e2, 0xb1eb5a08, 0x9cc7fbdf, 0x65b811ea, 0x41fd5ed9,
    0x2e6a6782, 0x3549661d, 0xbb211240, 0x78daa2ae, 0x8ce2d11f, 0x52911493, 0xc2497bd5, 0x83c232dd, 0x3e413e9f, 0x8831d191, 0x6770ac67,
    0xcd1c9141, 0xad35861a, 0xb79cd83d, 0xce3ec91f, 0x360942d1, 0x905000fa, 0x28bb469a, 0xdb239a17, 0x615cf3ae, 0xec9f7807, 0x271dcc3c,
    0x47b98e44, 0x33ff4a71, 0x02a063f8, 0xb051ebf2, 0x6f938d98, 0x2279abc3, 0xd55b01db, 0xaa99e301, 0x95d0587c, 0xaee8684e, 0x24574971,
    0x4b1e79a6, 0x4a646938, 0xa68d67f4, 0xb87839e6, 0x8e3d388b, 0xed2af964, 0x541b83e3, 0xcb7fc8da, 0xe1140f8c, 0xe9724fd6, 0x616a78fa,
    0x610cd51c, 0x10f9173e, 0x8e180857, 0xa8f0b843, 0xd429a973, 0xceee91e5, 0x1d4c6b18, 0x2a80e6df, 0x396f4d23,
};
...
int sketch_allocate(map_key_hash *kh, uint32_t capacity, uint16_t threshold, struct Sketch **sketch_out) {
  ...
  unsigned total_sketch_capacity = find_next_power_of_2_bigger_than(capacity * SKETCH_HASHES);

  (*sketch_out)->clients = NULL;
  if (map_allocate(hash_eq, hash_hash, total_sketch_capacity, &((*sketch_out)->clients)) == 0) {
    return 0;
  }

  (*sketch_out)->keys = NULL;
  if (vector_allocate(sizeof(struct hash), total_sketch_capacity, hash_allocate, &((*sketch_out)->keys)) == 0) {
    return 0;
  }

  (*sketch_out)->buckets = NULL;
  if (vector_allocate(sizeof(struct bucket), total_sketch_capacity, NULL, &((*sketch_out)->buckets)) == 0) {
    return 0;
  }
...
void sketch_compute_hashes(struct Sketch *sketch, void *key) {
  for (int i = 0; i < SKETCH_HASHES; i++) {
    ...
    sketch->internal.hashes[i] = __builtin_ia32_crc32si(sketch->internal.hashes[i], SKETCH_SALTS[i]);
    sketch->internal.hashes[i] = __builtin_ia32_crc32si(sketch->internal.hashes[i], sketch->kh(key));
...
void sketch_refresh(struct Sketch *sketch, time_t now) {
  for (int i = 0; i < SKETCH_HASHES; i++) {
    map_get(sketch->clients, &sketch->internal.hashes[i], &sketch->internal.buckets_indexes[i]);
    dchain_rejuvenate_index(sketch->allocators[i], sketch->internal.buckets_indexes[i], now);
  }
}
...
int sketch_fetch(struct Sketch *sketch) {
  ...
  for (int i = 0; i < SKETCH_HASHES; i++) {
    sketch->internal.present[i] = map_get(sketch->clients, &sketch->internal.hashes[i], &sketch->internal.buckets_indexes[i]);
...
int sketch_touch_buckets(struct Sketch *sketch, time_t now) {
  for (int i = 0; i < SKETCH_HASHES; i++) {
    int bucket_index = -1;
    int present      = map_get(sketch->clients, &sketch->internal.hashes[i], &bucket_index);

    if (!present) {
      int allocated_client = dchain_allocate_new_index(sketch->allocators[i], &bucket_index, now);
...
uint16_t ipv4_udptcp_cksum(const struct rte_ipv4_hdr *ipv4_hdr, const void *l4_hdr) {
  uint32_t cksum;
...
bool rte_ether_addr_eq(void *a, void *b) {
  ...
  return (id1->addr_bytes[0] == id2->addr_bytes[0]) && (id1->addr_bytes[1] == id2->addr_bytes[1]) &&
         (id1->addr_bytes[2] == id2->addr_bytes[2]) && (id1->addr_bytes[3] == id2->addr_bytes[3]) &&
         (id1->addr_bytes[4] == id2->addr_bytes[4]) && (id1->addr_bytes[5] == id2->addr_bytes[5]);
}
...
bool nf_init(void);
int nf_process(uint16_t device, uint8_t *buffer, uint16_t packet_length, time_t now);
...
static int nf_init_device(uint16_t device, struct rte_mempool **mbuf_pools) {
  ...
  for (int txq = 0; txq < num_queues; txq++) {
    retval = rte_eth_tx_queue_setup(device, txq, TX_QUEUE_SIZE, rte_eth_dev_socket_id(device), NULL);
    if (retval != 0) {
      return retval;
    }
  }
  ...
  RTE_LCORE_FOREACH(lcore_id) {
    ...
    retval = rte_eth_rx_queue_setup(device, rxq, RX_QUEUE_SIZE, rte_eth_dev_socket_id(device), NULL, mbuf_pools[rxq]);
    if (retval != 0) {
      return retval;
    }
...
static void worker_main(void) {
  ...
  while (1) {
    ...
    for (uint16_t DEVICE = 0; DEVICE < DEVICES_COUNT; DEVICE++) {
      ...
      uint16_t sent_count = rte_eth_tx_burst(1 - DEVICE, queue_id, mbufs_to_send, tx_count);
...
int main(int argc, char **argv) {
  ...
  RTE_LCORE_FOREACH(lcore_id) {
    ...
    mbuf_pools[lcore_idx] = rte_pktmbuf_pool_create(MBUF_POOL_NAME,                    // name
  ...
  RTE_LCORE_FOREACH_WORKER(lcore_id) { rte_eal_remote_launch((lcore_function_t *)worker_main, NULL, lcore_id); }
...
bool FlowId_eq(void *a, void *b) {
  ...
  return (id1->src_port == id2->src_port) && (id1->dst_port == id2->dst_port) && (id1->src_ip == id2->src_ip) &&
         (id1->dst_ip == id2->dst_ip) && (id1->protocol == id2->protocol);
}
...
uint8_t hash_key_0[RSS_HASH_KEY_LENGTH] = {0xa1, 0x24, 0x0,  0x15, 0x0,  0x14, 0xa1, 0x24, 0xa1, 0x24, 0x0,  0x14, 0xa1,
                                           0x24, 0x0,  0x15, 0xa7, 0xfa, 0x11, 0x22, 0x6f, 0xd3, 0xf0, 0x42, 0x1b, 0x6c,
                                           0xeb, 0x14, 0x62, 0x2,  0xa3, 0x44, 0x24, 0x90, 0xf8, 0x1c, 0x43, 0x99, 0xe7,
                                           0xaf, 0x80, 0x73, 0x15, 0xfe, 0x29, 0x5a, 0x73, 0xd0, 0x55, 0x85, 0xf2, 0xc4};
uint8_t hash_key_1[RSS_HASH_KEY_LENGTH] = {0x0,  0x14, 0xa1, 0x24, 0xa1, 0x24, 0x0,  0x15, 0x0,  0x14, 0xa1, 0x24, 0x0,
                                           0x14, 0xa1, 0x24, 0x6a, 0xe3, 0xac, 0x86, 0x3e, 0xcb, 0x7e, 0x73, 0x83, 0x15,
                                           0xcb, 0x75, 0xc4, 0x73, 0x2c, 0xda, 0xdb, 0x5,  0x31, 0x46, 0xdb, 0xd4, 0x76,
                                           0x5a, 0xa8, 0x20, 0x9d, 0xa,  0x44, 0x7a, 0xc6, 0xae, 0x5d, 0x72, 0x34, 0x9c};

struct rte_eth_rss_conf rss_conf[MAX_NUM_DEVICES] = {
    {.rss_key = hash_key_0, .rss_key_len = RSS_HASH_KEY_LENGTH, .rss_hf = RTE_ETH_RSS_NONFRAG_IPV4_TCP | RTE_ETH_RSS_NONFRAG_IPV4_UDP},
    {.rss_key = hash_key_1, .rss_key_len = RSS_HASH_KEY_LENGTH, .rss_hf = RTE_ETH_RSS_NONFRAG_IPV4_TCP | RTE_ETH_RSS_NONFRAG_IPV4_UDP}};
...
void report() {
  ...
  RTE_LCORE_FOREACH(lcore_id) {
    ...
    LOG("[%u] allocated %lu expired %lu", lcore_id, churns[lcore_id].allocated_flows, churns[lcore_id].expired_flows);
  }
...
bool nf_init() {
  ...
  if (!vector_allocate(sizeof(struct FlowId), FLOW_CAPACITY, FlowId_allocate, flows_values)) {
    return false;
  }
...
int nf_process(uint16_t device, uint8_t *packet, uint16_t packet_length, int64_t now) {
  ...
  int num_expired_flows = expire_items_single_map(flows_heap, flows_values, flows_map, last_time);
  ...
  if ((rte_be_to_cpu_16(ether_hdr->ether_type) != RTE_ETHER_TYPE_IPV4) || (packet_length < sizeof(struct rte_ipv4_hdr))) {
    // drop
    LOG_DEBUG("[core=%u,dev=%u] Not IP (ethertype 0x%04x)", rte_lcore_id(), device, rte_be_to_cpu_16(ether_hdr->ether_type));
    return DROP;
  }
  ...
  if (no_space) {
    // drop
    LOG_DEBUG("[core=%u,dev=%u] No more space in dchain", rte_lcore_id(), device);
    return device;
  }

  LOG_DEBUG("[core=%u,dev=%u] Allocated new flow with index %u", rte_lcore_id(), device, flow_index);
